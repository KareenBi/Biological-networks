...
def update_status(graph_status, com_status):
    max_com = max(graph_status.node2com.values())
    for node in com_status.node2com:
        graph_status.node2com[node] = max_com+com_status.node2com[node]+1
    # deg = float(graph.degree(node, weight=weight))
    # graph_status.degrees
    # self.gdegrees = dict([])
    # self.internals = dict([])
    pass
...
def __remove(node, com, weight, status):
    ...
    # status.com2node[com].remove(node)
