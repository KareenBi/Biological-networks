...
__PASS_MAX = -1
__MIN = 0.0000001
global disconnected_communities
...
def generate_dendrogram(graph,
    ...
    """counting disconnected communities: """
...
def run_code(edges_path, n):
    ...
    for i in range(n):
        ...
        avg_mod += modularity(partition, G)
        """counting disconnected communities: """
        com2node = generate_com2node(partition)
        disconnected = 0
        for com in com2node:
            com_nodes = com2node[com]
            com_graph = G.subgraph(com_nodes).copy()
            if(not nx.is_connected(com_graph)):
                print("!")
                print(len(com2node))
                disconnected += 1
        print(round(disconnected/len(com2node), 5), " of the communities are disconnected")
        """ --------------------------------- """
    print("The average number of disconnected communities produces by louvain is", round(disconnected_communities/n, 3), "disconnected communities")
    print("average modularity is: ", round(avg_mod/n, 3))
    return
...
com_1000 = ("/Users/kareen/Desktop/Semester_8/Biological_Networks/Graphs/10000_0.6_2/community.dat")
