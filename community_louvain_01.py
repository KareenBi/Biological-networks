...
def generate_dendrogram(graph,
    ...
    for com in com2node:
        ...
        f.write(str(nx.is_connected(com_graph)))
        com_status = Status()
...
def one_level_refine(com_graph, status, weight_key, resolution, random_state):
    """ Refinement step
    graph: a community of the graph
    ...
    in each iteration, we only move a node if (both):
    1. the modularity improves
    2. it's old community stays a connected graph
    """
    if(not nx.is_connected(com_graph)):
        print("unconnected com")
    modified = True
    nb_pass_done = 0
    new_mod = __modularity(status, resolution)
    #com2node = generate_com2node(status.node2com)
    while modified and nb_pass_done != __PASS_MAX:
        ...
        modified = False
        nb_pass_done += 1
        for node in __randomize(com_graph.nodes(), random_state):
            com_node = status.node2com[node]
            degc_totw = status.gdegrees.get(node, 0.) / (status.total_weight * 2.)  # NOQA
            neigh_communities = __neighcom(node, com_graph, status, weight_key)
            remove_cost = - neigh_communities.get(com_node,0) + \
                resolution * (status.degrees.get(com_node, 0.) - status.gdegrees.get(node, 0.)) * degc_totw
            com2node = generate_com2node(status.node2com)
            com_nodes = set(com2node[com_node])-{node}
            subgraph = com_graph.subgraph(com_nodes).copy()
