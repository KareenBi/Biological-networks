...
def generate_dendrogram(graph,
    ...
    for com in com2node:
        if(always_refine):
            ...
            com_graph = current_graph.subgraph(com_nodes)
...
def refinement_step(graph, status, com, weight_key, resolution, random_state):
    ...
    com2node = generate_com2node(status.node2com)
    com_nodes = com2node[com] 
    first_new_com = max(set(status.node2com.values()))+1 
    new_com = first_new_com
    ...
    while modified and nb_pass_done != __PASS_MAX:
        ...
        for node in __randomize(com_nodes, random_state):
            ...
            remove_cost = - neigh_communities.get(com_node,0) + \
                resolution * (status.degrees.get(com_node, 0.) - status.gdegrees.get(node, 0.)) * degc_totw
            subgraph = graph.subgraph(set(com2node[com_node])-{node})
...
def mod_leiden(edges_path):
    G = ig.Graph()
    G = ig.Graph.Read_Ncol(edges_path, directed = False)
    partition = la.find_partition(G, la.ModularityVertexPartition)
    print(round(partition.modularity,4))
...
def random_networks_test(cluster_path,edges_path):
    modularity_01, jaccard_01,com_num1, modularity_02, jaccard_02, com_num2 = 0,0,0,0,0,0
    for i in range(10):
        G = nx.read_edgelist(edges_path + str(i)+ r"\network.dat", delimiter = "\t")
        known_solution = generate_known_solution(cluster_path + str(i) + r"\community.dat")
        # calculations based on the original code
        partition_01 = community_louvain.best_partition(G)
        com_num1 +=len(set(partition_01.values()))
        ...
        com_num2 +=len(set(partition_02.values()))
        modularity_02 += modularity(partition_02, G)    
        jaccard_02 += __jaccard(known_solution, partition_02)
    modularity_01 = round(modularity_01/10, 5)
    jaccard_01 = round(jaccard_01/10, 5)
    com_num1 = round(com_num1/10)
    modularity_02 = round(modularity_02/10, 5)
    jaccard_02 = round(jaccard_02/10, 5)
    com_num2 = round(com_num2/10)
    print("Jaccard_01: ", jaccard_01)
    print("Modularity_01: ", modularity_01)
    print("com_num1: ", com_num1)
    print("\n")
    print("Jaccard_02: ", jaccard_02)
    print("Modularity_02: ", modularity_02)
    print("com_num2: ", com_num2)
    print("\n")
    if(modularity_01>modularity_02):
...
def mod_jacc_test(cluster_path, edges_path, n):
    ...
    for i in range(n):
        ...
        #calculations based on our extended code
        partition_02 = best_partition(G, always_refine=False)
        com_num2 += len(set(partition_02.values()))
        modularity_02 += modularity(partition_02, G)
        jaccard_02 += __jaccard(known_solution, partition_02)
...
def mod_test(edges_path, n):
    G = nx.read_edgelist(edges_path, delimiter = "\t")
    modularity_01, com_num1, com_num2, modularity_02 = 0, 0,0,0
    for i in range(n):
        # calculations based on the original code
        partition_01 = community_louvain.best_partition(G)
        com_num1 += len(set(partition_01.values()))
...
def leiden_random(edges_path):
    modularity = 0
    for i in range(10):
        G = ig.Graph()
        G = ig.Graph.Read_Ncol(edges_path + str(i)+ r"\network.dat", directed = False)
        partition = la.find_partition(G, la.ModularityVertexPartition)
        # calculations based on the original code
        partition = la.find_partition(G, la.ModularityVertexPartition)
        modularity += partition.modularity
    modularity = round(modularity/10, 5)
    print("Modularity_: ", modularity)
# ---------------------------------------------------------------------------------------------------------------------
yeast_edges = (r"G:\My Drive\Biological Networks\Benchmarks\Benchmarks\Yeast\edges.txt")
yeast_cluster = (r"G:\My Drive\Biological Networks\Benchmarks\Benchmarks\Yeast\clusters.txt")

arabidopsis_edges = (r"G:\My Drive\Biological Networks\Benchmarks\Benchmarks\Arabidopsis\edges.txt")        
arabidopsis_cluster = (r"G:\My Drive\Biological Networks\Benchmarks\Benchmarks\Arabidopsis\clusters.txt")

random_edges = (r"G:\My Drive\Biological Networks\Benchmarks\Benchmarks\Graphs\10000_0.6_")
random_cluster = (r"G:\My Drive\Biological Networks\Benchmarks\Benchmarks\Graphs\10000_0.6_")
...
mod_leiden(arabidopsis_edges)
# mod_test(dblp,1)
# leiden_random(random_edges)
# random_networks_test(random_cluster,random_edges)
# mod_jacc_test(yeast_cluster, yeast_edges, 1)
